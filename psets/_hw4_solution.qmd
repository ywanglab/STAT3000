...
This document provides a complete solution to Problem Set 6 in a single `.qmd` file. In a project-style submission, I would split the code across `code/funcs.R`, `code/wrangle.R`, and `code/analysis.qmd`, save the final weekly dataset to `data/dat.rds`, and save the figures to `figs/`. For convenience, everything is included inline here.
...
resp_content_type(population_response)
...
deaths <- deaths_raw |>
  ...
  mutate(state = coalesce(state, state_value)) |>
