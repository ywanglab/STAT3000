...
```{r}
    ...
    plot(x, fx, type="l")
    abline(h=0) #h=0: horizontal y-inercept; v=t: vertical line with x-intercept t; abline(a,b): y=a+bx
    points(c(x1,x2), c(0,0), col="red") # add ponts to an exsiting plot
...
6. **Write coefficients to a text file using Unix**
   ...
   ```
   # Your commands here
   printf "2 -5 -3" > ../data/coefs.txt
   # or 
   # echo "2 -5 -3" > ../data/ceofs.txt
   ```
...
7. **Copy the Quarto file using Unix**
   ...
   ```
   # Your commands here (assumng your curretn directory is the parent director of hw/ and code/)
   cp hw/hw1.qmd code/quadratic.qmd
   ```
...
# getwd(): get the abs. path of the current working directory
# file.path(): build full abs path that works in any os. Second argularment is relative to the first argument
...
```
  ...
  # first change directory to the parent directory that contants the entire poject directory (e.g. STAT3000)
  cp -r STAT3000 Rtmp  
  ```
